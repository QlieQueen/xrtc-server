...
public:
    ...
    void set_direction(RtpDirection d) { _direction = d; }
    ...
    void add_candidates(const std::vector<Candidate>& c) { _candidates = c; }
    const std::vector<StreamParams>& streams() { return _send_streams; }
    void add_stream(const StreamParams& s) { _send_streams.push_back(s); }
    void add_codec(std::shared_ptr<CodecInfo> codec) { _codecs.push_back(codec); }

protected:
    std::vector<std::shared_ptr<CodecInfo>> _codecs;
    RtpDirection _direction = RtpDirection::k_inactive;
    bool _rtcp_mux = true;
...
public:
    ...
    bool has_content_name(const std::string& name);
    void add_content_name(const std::string& name);
...
// -- TransportDescription (ICE + DTLS 传输信息) ---
enum ConnectionRole {
    NONE = 0,
    ACTIVE,
    PASSIVE,
